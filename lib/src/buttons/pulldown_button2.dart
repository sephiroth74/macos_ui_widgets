...
/// See also:
///
///  * [MacosPulldownMenuItem2], the class used to represent the menu options.
///  * [MacosPulldownMenuDivider], the class used to represent a horizontal
///    divider for the menu.
class MacosPulldownButton2<T> extends StatefulWidget {
  ...
  ///
  /// Can be either [MacosPulldownMenuItem2]s or [MacosPulldownMenuDivider]s.
  ///
...
class _MacosPulldownButton2State<T> extends State<MacosPulldownButton2<T>>
  ...
  void _handleTap() {
    ...
    navigator.push(_pulldownRoute!).then<void>((value) {
  ...
  Widget build(BuildContext context) {
    ...
    final baseStyle =
        theme.typography.body.copyWith(color: MacosColors.controlTextColor);
    final caretSize = _getCaretSize(widget.controlSize);
    final caretPadding = _getCaretPadding(_hasIcon, widget.controlSize);
    final caretStrokeSize = _getStrokeSize(widget.controlSize);
    final iconSize = _getIconsSize(widget.controlSize);

    Widget result = Container(
      constraints: _hasIcon
          ? widget.controlSize.constraints.copyWith(minWidth: 0)
          : widget.controlSize.constraints,
        ...
        children: <Widget>[
          _hasIcon
              ? MacosIcon(widget.icon!,
                  color: buttonStyles.textColor, size: iconSize)
              : _enabled
                  ? DefaultTextStyle(
                  ...
                  : DefaultTextStyle(
                      style: widget.controlSize
                          .textStyle(baseStyle)
                          .copyWith(color: buttonStyles.textColor),
                      child: Text(widget.disabledTitle ?? widget.title!),
                    ),
          Padding(
  ...
  double _getIconsSize(ControlSize controlSize) {
    switch (controlSize) {
  ...
  Size _getCaretSize(ControlSize controlSize) {
    switch (controlSize) {
      case ControlSize.mini:
        return const Size(8.0, 8.0);
      case ControlSize.small:
        return const Size(12.0, 10.0);
      case ControlSize.regular:
        return const Size(14.0, 14.0);
      case ControlSize.large:
        return const Size(14.0, 14.0);
    }
  }

  double _getStrokeSize(ControlSize controlSize) {
    switch (controlSize) {
      case ControlSize.mini:
        return 1;
      case ControlSize.small:
  ...
  EdgeInsetsGeometry _getCaretPadding(bool hasIcon, ControlSize controlSize) {
    switch (controlSize) {
      case ControlSize.mini:
        return EdgeInsets.only(left: hasIcon ? 2.0 : 2.0, top: 0.0);
      case ControlSize.small:
        return EdgeInsets.only(left: hasIcon ? 2.0 : 4.0, top: 0.0);
      case ControlSize.regular:
        return EdgeInsets.only(left: hasIcon ? 4.0 : 6.0, top: 2.0);
      case ControlSize.large:
        return EdgeInsets.only(left: hasIcon ? 6.0 : 8.0, top: 1.0);
    }
  }
}
...
_ButtonStyles _getButtonStyles(
  ...
  Color caretColor = MacosColors.black;
  if (!enabled) {
    if (hasIcon) {
...
class _DownCaretPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    /// Draw carets
    final p1 = Offset(size.width / 4, size.height / 2 - 1.0);
    final p2 = Offset(size.width / 2, size.height / 2 + 2.0);
    final p3 = Offset(size.width / 2 + 1, size.height / 2 + 1.0);
    final p4 = Offset(size.width / 2 + size.width / 4, size.height / 2 - 1.0);
    final paint = Paint()
      ..color = color
